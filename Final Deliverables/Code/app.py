...
app.config['hostname'] = 'b70af05b-76e4-4bca-a1f5-23dbb4c6a74e.c1ogj3sd0tgtu0lqde00.databases.appdomain.cloud' #b70af05b-76e4-4bca-a1f5-23dbb4c6a74e.c1ogj3sd0tgtu0lqde00.databases.appdomain.cloud
app.config['port'] = '32716' #32716
app.config['protocol'] = 'tcpip' #tcpip
app.config['uid'] = 'kxx08419' #kxx08419
app.config['pwd'] = 'xS05C4qZ44ixaYGy' #xS05C4qZ44ixaYGy
app.config['security'] = 'SSL' #SSL
try:
    mysql = DB2(app)

    conn_str = 'database=bludb;hostname=b70af05b-76e4-4bca-a1f5-23dbb4c6a74e.c1ogj3sd0tgtu0lqde00.databases.appdomain.cloud;port=32716;protocol=tcpip; uid=kxx08419;pwd=xS05C4qZ44ixaYGy;security=SSL'
...
def today():
    param1 = "SELECT TIME(date) as tn, amount FROM expenses WHERE userid = " + str(
        session['id']) + " AND DATE(date) = DATE(current timestamp) ORDER BY date DESC"
...
@app.route("/month")
def month():
    param1 = "SELECT DATE(date) as dt, SUM(amount) as tot FROM expenses WHERE userid = " + str(session[
...
def year():
    param1 = "SELECT MONTH(date) as mn, SUM(amount) as tot FROM expenses WHERE userid = " + str(
        session['id']) + " AND YEAR(date) = YEAR(current timestamp) GROUP BY MONTH(date) ORDER BY MONTH(date)"
    res1 = ibm_db.exec_immediate(ibm_db_conn, param1)
    dictionary1 = ibm_db.fetch_assoc(res1)
    texpense = []
    ...
    param = "SELECT * FROM expenses WHERE userid = " + str(
        session['id']) + " AND YEAR(date) = YEAR(current timestamp) ORDER BY date DESC"
    res = ibm_db.exec_immediate(ibm_db_conn, param)
    dictionary = ibm_db.fetch_assoc(res)
    expense = []
    while dictionary != False:
